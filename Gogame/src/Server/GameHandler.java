...
/**
 * The game class is responsible for the gameflow and enforcing the rules of a
 * game of go. Two clienthandlers are assigned to this game by the server and
 * the game is responsible for communicating with these clients.
 * 
 * @author luuk.bosch
 *
 */
public class GameHandler {
	...
	/**
	 * Handles all the incoming messages of the clients. Only messages obeying the
	...
	public void handleExit(ClientHandler player) {
		...
		if (!getOpponent(player).isStopped()) {
		messageSender.sendExitMessage(player);
		getOpponent(player).GameOver();
		}
